...
struct _MooPanedPrivate {
    ...
    /* XXX weak pointer */
    gpointer     focus_child; /* focused grandchild of gtk_bin_get_child (GTK_BIN (paned)) */
    MooPane     *focus_pane;
    FocusPosition focus;
...
static void moo_paned_get_preferred_width(GtkWidget*,gint*,gint*);
static void moo_paned_get_preferred_height(GtkWidget*,gint*,gint*);
static void     moo_paned_size_allocate     (GtkWidget      *widget,
                                             GtkAllocation  *allocation);

static gboolean moo_paned_expose            (GtkWidget      *widget, cairo_t *cr);
                      ...
                      GdkRectangle *rect)
{
    ...
    switch (paned->priv->pane_position)
    {
        case MOO_PANE_POS_LEFT:
            ...
            break;
        case MOO_PANE_POS_RIGHT:
            rect->x += moo_widget_get_alloc(GTK_WIDGET(paned)).width - rect->width -
        ...
        case MOO_PANE_POS_TOP:
            ...
            break;
        case MOO_PANE_POS_BOTTOM:
            rect->y += moo_widget_get_alloc(GTK_WIDGET(paned)).height - rect->height -
                            paned->priv->button_box_size;
            break;
    }
}
                        ...
                        GtkRequisition *requisition)
{
    ...
    if (gtk_bin_get_child (GTK_BIN (paned)) && gtk_widget_get_visible (GTK_WIDGET (gtk_bin_get_child (GTK_BIN (paned)))))
    {
        gtk_widget_get_preferred_size (gtk_bin_get_child (GTK_BIN (paned)), &child_requisition, NULL);
        requisition->width += child_requisition.width;
        requisition->height += child_requisition.height;
    }
    ...
    if (paned->priv->enable_border)
    {
        switch (paned->priv->pane_position)
        {
            case MOO_PANE_POS_LEFT:
            case MOO_PANE_POS_RIGHT:
                paned->priv->border_size = 1 /* GTK3: use CSS padding instead */;
                            ...
                            GtkAllocation   *allocation)
{
    allocation->x = 0;
    allocation->y = 0;
    allocation->width = moo_widget_get_alloc(GTK_WIDGET(paned)).width;
    allocation->height = moo_widget_get_alloc(GTK_WIDGET(paned)).height;
                           ...
                           GtkAllocation   *allocation)
{
    ...
    switch (paned->priv->pane_position)
    {
        case MOO_PANE_POS_LEFT:
            ...
            break;
        case MOO_PANE_POS_RIGHT:
            allocation->x = moo_widget_get_alloc(GTK_WIDGET(paned)).width - allocation->width;
            break;
        case MOO_PANE_POS_TOP:
            allocation->y = 0;
            break;
        case MOO_PANE_POS_BOTTOM:
            allocation->y = moo_widget_get_alloc(GTK_WIDGET(paned)).height - allocation->height;
            break;
    }
}
    ...
    switch (paned->priv->pane_position)
    {
        ...
        case MOO_PANE_POS_RIGHT:
            paned->priv->handle_size = CLAMP (paned->priv->handle_size, 0,
                                              moo_widget_get_alloc(GTK_WIDGET(paned)).width);
            break;
        case MOO_PANE_POS_TOP:
        case MOO_PANE_POS_BOTTOM:
            paned->priv->handle_size = CLAMP (paned->priv->handle_size, 0,
                                              moo_widget_get_alloc(GTK_WIDGET(paned)).height);
            break;
    }
}
                        ...
                        GtkRequisition *child_requisition)
{
    ...
    switch (paned->priv->pane_position)
    {
        case MOO_PANE_POS_LEFT:
        case MOO_PANE_POS_RIGHT:
            max_size = moo_widget_get_alloc(GTK_WIDGET(paned)).width -
                                      paned->priv->handle_size -
                                      paned->priv->button_box_size;
            if (paned->priv->sticky)
        ...
        case MOO_PANE_POS_TOP:
        case MOO_PANE_POS_BOTTOM:
            max_size = moo_widget_get_alloc(GTK_WIDGET(paned)).height -
                                      paned->priv->handle_size -
                                      paned->priv->button_box_size;
            if (paned->priv->sticky)
                        ...
                        GdkRectangle  *rect)
{
    switch (paned->priv->pane_position)
    {
        ...
        case MOO_PANE_POS_RIGHT:
            ...
            rect->height = moo_widget_get_alloc(GTK_WIDGET(paned)).height;
            break;
        case MOO_PANE_POS_TOP:
        case MOO_PANE_POS_BOTTOM:
            rect->x = 0;
            rect->height = paned->priv->handle_size;
            rect->width = moo_widget_get_alloc(GTK_WIDGET(paned)).width;
            break;
    }
                         ...
                         GtkAllocation *allocation)
{
    ...
    if (gtk_bin_get_child (GTK_BIN (paned)))
    {
        ...
        gtk_widget_size_allocate (gtk_bin_get_child (GTK_BIN (paned)), &child_allocation);
    }
                  ...
                  gpointer        callback_data)
{
    ...
    if (gtk_bin_get_child (GTK_BIN (paned)))
        callback (gtk_bin_get_child (GTK_BIN (paned)), callback_data);
...
static gboolean
moo_paned_expose (GtkWidget      *widget, cairo_t *cr)
{
    MooPaned *paned = MOO_PANED (widget);
    ...
    if (gtk_bin_get_child (GTK_BIN (paned)) && gtk_widget_is_drawable (gtk_bin_get_child (GTK_BIN (paned))))
        gtk_container_propagate_draw (GTK_CONTAINER (paned),
                                        gtk_bin_get_child (GTK_BIN (paned)), cr);
    ...
    /* Manual button-strip / notebook separator removed: GTK3 themes already
     * draw their own borders on the adjacent widgets, so this would render
     * as a stray 1px line. */
...
#if 0
/* TODO */
static cairo_t *clip_bin_child_event (MooPaned       *paned,
                                             cairo_t *event)
{
    GtkAllocation child_alloc;
    cairo_region_t *child_rect;
    cairo_t *child_event;

    get_bin_child_allocation (paned, &child_alloc);
    child_rect = cairo_region_create_rectangle ((GdkRectangle*) &child_alloc);

    child_event = (cairo_t *) gdk_event_copy ((GdkEvent*) event);
    gdk_region_intersect (child_event->region, child_rect);
    gdk_region_get_clipbox (child_event->region, &child_event->area);

    cairo_region_destroy (child_rect);
    return child_event;
}
#endif
...
draw_handle (MooPaned *paned, cairo_t *cr)
{
    ...
    switch (paned->priv->pane_position)
    {
        ...
        case MOO_PANE_POS_RIGHT:
            ...
            area.height = moo_widget_get_alloc(widget).height;
        ...
        case MOO_PANE_POS_BOTTOM:
            shadow_size = 1;
            area.width = moo_widget_get_alloc(widget).width;
    ...
    /* Manual bevel lines around the handle removed: GTK3 themes draw their
     * own borders on the adjacent pane and notebook widgets. */
        ...
        switch (paned->priv->pane_position)
        {
            ...
            case MOO_PANE_POS_RIGHT:
                ...
                if (paned->priv->pane_position == MOO_PANE_POS_RIGHT)
                    size = moo_widget_get_alloc(widget).width - size;

                size -= (paned->priv->drag_start + paned->priv->button_box_size);
                size = CLAMP (size, requisition.width,
                              moo_widget_get_alloc(widget).width - paned->priv->button_box_size -
                                      paned->priv->handle_size);
                break;
            ...
            case MOO_PANE_POS_BOTTOM:
                ...
                if (paned->priv->pane_position == MOO_PANE_POS_BOTTOM)
                    size = moo_widget_get_alloc(widget).height - size;

                size -= (paned->priv->drag_start + paned->priv->button_box_size);
                size = CLAMP (size, requisition.height,
                              moo_widget_get_alloc(widget).height - paned->priv->button_box_size -
                                      paned->priv->handle_size);
                break;
        }
                                ...
                                GdkWindow    *reference)
{
    ...
    else if (last_button)
    {
        ...
        GtkAllocation lba;
        gtk_widget_get_allocation (last_button, &lba);
        rect->x = lba.x;
        rect->y = lba.y;
        rect->width = lba.width;
        rect->height = lba.height;
        ...
        switch (paned->priv->pane_position)
        {
            ...
            case MOO_PANE_POS_RIGHT:
                ...
                rect->height = MIN (60, moo_widget_get_alloc(widget).height - rect->y);
                rect->height = MAX (rect->height, 10);
                break;
            case MOO_PANE_POS_TOP:
            case MOO_PANE_POS_BOTTOM:
                rect->x += rect->width + button_spacing;
                rect->width = MIN (60, moo_widget_get_alloc(widget).width - rect->x);
    ...
    else
    {
        ...
        {
            case MOO_PANE_POS_RIGHT:
                rect->x = moo_widget_get_alloc(widget).width - 30;
                // fallthrough
            case MOO_PANE_POS_LEFT:
                rect->height = 60;
                rect->width = 30;
                break;
            case MOO_PANE_POS_BOTTOM:
                rect->y = moo_widget_get_alloc(widget).height - 30;
                // fallthrough
            case MOO_PANE_POS_TOP:
                rect->width = 60;
                rect->height = 30;
                              ...
                              int         size)
{
    ...
    rect.x = moo_widget_get_alloc(widget).x;
    rect.y = moo_widget_get_alloc(widget).y;
    rect.width = moo_widget_get_alloc(widget).width;
    rect.height = moo_widget_get_alloc(widget).height;
...
{
    gboolean visible = gtk_widget_get_visible (GTK_WIDGET (paned->button_box));
               ...
               MooPaned       *paned)
{
    ...
    alloc = moo_widget_get_alloc (widget);
    height = MIN (alloc.height, HANDLE_HEIGHT);
...
#if 0
    cursor = gdk_cursor_new_for_display (gdk_display_get_default (), paned->priv->handle_cursor_type);
